...
const GoogleSheets = observer((props) => {
	const [googleSheetsData] = React.useState(
		is.falsy(KaliFormsGoogleSheets.sheets)
			? { error: "Something went wrong" }
			: KaliFormsGoogleSheets.sheets
	);
	...
	const selectedSpreadSheet = React.useMemo(() => {
		let spread = googleSheetsData.spreadsheets.filter(
			(spread) => spread.id === store._GOOGLE_SHEETS_.spreadsheet
		);
		return spread.length > 0 ? spread[0] : undefined;
	}, [store._GOOGLE_SHEETS_]);

	React.useEffect(() => {
		if (
			typeof store._GOOGLE_SHEETS_.sheet === "undefined" ||
			store._GOOGLE_SHEETS_.sheet === "kf-select-field" ||
			is.empty(store._GOOGLE_SHEETS_.sheet)
		) {
			return;
		}
		...
		setLoading(true);
		axios
			.post(KaliFormsObject.ajaxurl, Qs.stringify(data))
			.then((r) => {
									...
									<FormControlLabel
										control={
											<Checkbox
												...
												onChange={(e) =>
													store._GOOGLE_SHEETS_.setEnabled(
									...
									<FormControl>
										...
										<Select
											...
											onChange={(e) => {
												...
												store._GOOGLE_SHEETS_.setSelectedSpreadsheet(
													e.target.value
												);
											}}
									...
									<Grid item xs={6}>
										<FormControl>
											...
											<Select
												...
												onChange={(e) => {
													store._GOOGLE_SHEETS_.fields = {};
													store._GOOGLE_SHEETS_.setSelectedSheet(
														e.target.value
													);
												}}
												input={<BootstrapInput />}
								...
								<Grid container direction="row" spacing={3}>
									{cells.map((field) => (
										<Grid
											item
											xs={6}
											key={sanitizeName(field.index, field.name)}
										>
											<FieldComponentSelect
												...
												selectedValue={
													store._GOOGLE_SHEETS_.fields[
														sanitizeName(field.index, field.name)
													] || ""
												}
