...
class LknPaymentPixForWoocommerceHelper
{
    ...
    public function showOrderLogs(): void
    {
        ...
        if (! empty($id)) {
            ...
            if ($order && $order instanceof WC_Order) {
                $orderLogs = $order->get_meta('lknPaymentPixGatewayOrderLogs');
                $payment_method_id = $order->get_payment_method();
                $options = get_option('woocommerce_' . $payment_method_id . '_settings');
                if ($orderLogs && 'yes' === $options['show_order_logs'] && $this->isValidGatewayId($payment_method_id)) {
                    //carregar css
                    wp_enqueue_style('lkn-pix-for-woocommerce-order-logs', plugin_dir_url(dirname(__FILE__)) . 'Public/css/lkn-admin-order-logs.css', array(), PAYMENT_PIX_FOR_WOOCOMMERCE_VERSION, 'all');
...
?>
            <div id="lknPaymentPixGatewayOrderLogs">
                <div>
                    <h3>URL:</h3>
...
<?php
    ...
    public function isValidGatewayId($gateway_id): bool
    {
        if (is_array(PAYMENT_PIX_FOR_WOOCOMMERCE_GATEWAY_IDS)) {
            $valid_gateway_ids = PAYMENT_PIX_FOR_WOOCOMMERCE_GATEWAY_IDS;
        } else {
            // Se for string JSON, decodificar diretamente
            $valid_gateway_ids = json_decode(PAYMENT_PIX_FOR_WOOCOMMERCE_GATEWAY_IDS, true);
        }
        return in_array($gateway_id, $valid_gateway_ids, true);
    }
}
